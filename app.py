...
def render_map(path, poly_gdf=None, line_gdf=None, pts_gdf=None,
    ...
    Standard mode (slope_mode=False):
      - A4 portrait, 300 DPI, white bg
      - UTM grid ticks on all 4 sides
      - Blue boundary (no fill), red survey dots with SN labels
      - Polygon at ~70% of frame (21% margin each side)
      - NO baked north arrow or scale bar — those are HTML overlays in the UI

    Slope mode (slope_mode=True):
      - Same but with baked north arrow + scale bar + slope table
      - Used for Group F / Group H maps
    """
    # ── SLOPE MODE ──────────────────────────────────────────────────────
    if slope_mode:
        fig = plt.figure(figsize=(FIG_W, FIG_H), dpi=DPI)
        fig.patch.set_facecolor(MAP_BG)
        sr = safe_rect if safe_rect else (0.10, 0.10, 0.82, 0.82)
        gs = gridspec.GridSpec(2, 1, height_ratios=[1, 0.18],
                               left=sr[0], right=sr[0]+sr[2],
                               bottom=sr[1], top=sr[1]+sr[3], hspace=0.15)
        ax = fig.add_subplot(gs[0])
        ax_tbl = fig.add_subplot(gs[1]); ax_tbl.axis('off')
        ax.set_facecolor(MAP_BG); ax.axis('off')
        _plot_polygons(ax, poly_gdf, label_col, [], fig)
        _plot_lines(ax, line_gdf)
        _plot_points(ax, pts_gdf, point_label_col, [], fig)
        gdfs = [g for g in [poly_gdf, line_gdf, pts_gdf] if g is not None and not g.empty]
        if gdfs:
            bounds = _get_combined_bounds(gdfs)
            if bounds: _fit_bounds_to_axes(ax, bounds, sr, FIG_W, FIG_H)
        if slope_areas: _draw_slope_table(ax, slope_areas)
        if title: fig.suptitle(title, fontsize=12, weight='bold')
        _add_north_arrow(fig, pos=(0.93, 0.95))
        _add_scale_bar(fig, ax)
        fig.savefig(path, dpi=DPI, facecolor=MAP_BG, bbox_inches='tight')
...
def _g_preview(shp_gdf, poly_gdf, path, safe_rect=None, title="Forest Survey Points", layout_state=None):
    render_map(path, poly_gdf=poly_gdf, pts_gdf=shp_gdf,
               point_label_col="Point_ID", title=title)
